...
resource "random_uuid" "ck_file_uuid" {
}

resource "aws_sfn_state_machine" "ck_sfn_state_machine" {
  name     = var.ck_sfn_state_machine_name
  role_arn = aws_iam_role.ck_step_functions_master_restricted_access[0].arn
  definition = jsonencode({
    Comment = "A State Machine that processes files with Amazon Transcribe Call Analytics"
    StartAt = var.ck_sfn_state_generate_uuid_name
    States = merge(
      # - Generate UUID -
      # Generates a UUID from S3 PutObject id
      {
        # (var.ck_sfn_state_generate_uuid_name) = {
        "${var.ck_sfn_state_generate_uuid_name}" = {
          Type = "Pass",
          Result = {
            "uuid.$" = "$.id"
          ...
          Next       = var.ck_sfn_state_generate_uuid_next_step
        }
      },

      # - StartCallAnalyticsJob -
      # Starts an Amazon Transcribe Call Analytics Job with the file copied to ck_input_bucket
      {
        # (var.ck_sfn_state_start_call_analytics_job_name) = {
        "${var.ck_sfn_state_start_call_analytics_job_name}" = {
          Type     = "Task",
          Resource = "arn:aws:states:::aws-sdk:transcribe:startCallAnalyticsJob",
          Comment  = "Starts an Amazon Transcribe Call Analytics Job with the file copied to ck_input_bucket"
          ...
          Parameters = merge(
              ...
              ChannelDefinitions = [
                {
                  ...
                  ParticipantRole = var.ck_first_speaker_participant_role
                },
                {
                  ChannelId       = 1, // next person to speak in the call
                  ParticipantRole = var.ck_second_speaker_participant_role
                }
              ]
            },
            {
              "OutputLocation.$" = "States.Format('s3://${aws_s3_bucket.ck_output_bucket.id}/{}-{}/{}-{}-transcript.json' ,$.detail.object.key ,$.id ,$.detail.object.key, $.id)"
            },
            {
              Settings = merge(
                {
                  # (common attributes here)
                  LanguageOptions = var.ck_language_options
                },
                # Content Redaction
                coalesce(var.ck_enable_content_redaction ? {
                  # (conditional attributes here)
                  ContentRedaction = {
                    RedactionType   = "PII"
                    RedactionOutput = var.ck_use_redaction_type_redacted_and_unredacted ? "redacted_and_unredacted" : "redacted"
                    PiiEntityTypes  = var.ck_pii_entity_types
                  }
                } : null, {}),

                # Custom Vocabulary
                coalesce(var.ck_use_custom_vocabulary ? {
                  # (conditional attributes here)
                  VocabularyName = var.ck_custom_vocabulary_name
                } : null, {}),
                coalesce(var.ck_use_custom_vocabulary_filter ? {
                  # (conditional attributes here)
                  VocabularyFilterName = var.ck_custom_vocabulary_filter_name
                } : null, {}),

                # Custom Language Model
                coalesce(var.ck_use_custom_language_model ? {
                  # (conditional attributes here)
                  LanguageModelName = var.ck_custom_language_model_name
                } : null, {}),

                # Language ID Settings
                coalesce(var.ck_use_language_id_settings ? {
                  # (conditional attributes here)
                  LanguageIdSettings = {
                    VocabularyName       = var.ck_use_custom_vocabulary ? var.ck_custom_vocabulary_name : null
                    VocabularyFilterName = var.ck_use_custom_vocabulary_filter ? var.ck_custom_vocabulary_filter_name : null
                    LanguageModelName    = var.ck_use_custom_language_model ? var.ck_custom_language_model_name : null
            ...
            coalesce(!(var.ck_enable_content_redaction) ? {
              # (conditional attributes here)
              Media = {
                "MediaFileUri.$" = "States.Format('s3://${aws_s3_bucket.ck_input_bucket.id}/{}-{}',$.detail.object.key, $.id)"
              }
            ...
            coalesce((var.ck_enable_content_redaction) ? {
              ...
              Media = {
                "RedactedMediaFileUri.$" = "States.Format('s3://${aws_s3_bucket.ck_input_bucket.id}/{}-{}',$.detail.object.key, $.id)"
              }
            } : null, {}),
          )
      ...
      # - CopyToAppStorage -
      {
        CopyToAppStorage = {
          ...
          Parameters = {
            ...
            "CopySource.$" = "States.Format('${aws_s3_bucket.ck_input_bucket.id}/{}',$.CallAnalyticsJob.CallAnalyticsJobName)"         // reference bucket name and object path from InputPath
          },
          Next = "GetTCAMetadata"
        }
      },
      ...
      # - WriteToDynamoDB -
      {
        WriteToDynamoDB = {
          ...
          ResultPath = "$.DynamoDB"
          Parameters = {
            TableName = "${aws_dynamodb_table.ck_output.id}"
            Item = {
              # Column = {
              #   S = "JobName"
              # }
      ...
      # - TCAFailSNS -
      {
        TCAFailSNS = {
          ...
          Parameters = {
            ...
            TopicArn = "${aws_sns_topic.ck_sfn_status.arn}"
          }
          Next = "Fail"
        }
      },
      ...
      coalesce((var.create_ck_sfn_state_get_ck_input_file) ? {
        # (var.ck_sfn_state_get_ck_input_file_name) = {
        "${var.ck_sfn_state_get_ck_input_file_name}" = {
          Type       = "Task",
          Resource   = "arn:aws:states:::aws-sdk:s3:copyObject",
