...
# API Data Source
resource "aws_appsync_datasource" "ck_appsync_dynamodb_datasource" {
  api_id           = aws_appsync_graphql_api.ck_appsync_graphql_api.id
  name             = "ck_output_dynamodb_datasource"
  service_role_arn = aws_iam_role.ck_appsync_dynamodb_restricted_access[0].arn
  type             = "AMAZON_DYNAMODB"

  dynamodb_config {
    table_name = aws_dynamodb_table.ck_output.name
  }
}
# API
resource "aws_appsync_graphql_api" "ck_appsync_graphql_api" {
  authentication_type = "AMAZON_COGNITO_USER_POOLS"
  name                = var.ck_appsync_graphql_api_name
...
# Query - Get One Job
resource "aws_appsync_resolver" "ck_appsync_resolver_query_get_one_job" {
  api_id = aws_appsync_graphql_api.ck_appsync_graphql_api.id
  field  = "getOneJob"
  type   = "Query"
  # data_source = [aws_appsync_datasource.ck_appsync_dynamodb_datasource.name]
  data_source = aws_appsync_datasource.ck_appsync_dynamodb_datasource.name
...
resource "aws_appsync_resolver" "ck_appsync_resolver_query_get_all_jobs" {
  api_id      = aws_appsync_graphql_api.ck_appsync_graphql_api.id
  field       = "getAllJobs"
  type        = "Query"
  data_source = aws_appsync_datasource.ck_appsync_dynamodb_datasource.name
...
resource "aws_appsync_resolver" "ck_appsync_resolver_query_get_all_jobs_paginated" {
  api_id      = aws_appsync_graphql_api.ck_appsync_graphql_api.id
...
resource "aws_appsync_resolver" "ck_appsync_resolver_mutation_delete_one_job" {
  api_id      = aws_appsync_graphql_api.ck_appsync_graphql_api.id
  field       = "deleteOneJob"
  type        = "Mutation"
  data_source = aws_appsync_datasource.ck_appsync_dynamodb_datasource.name
